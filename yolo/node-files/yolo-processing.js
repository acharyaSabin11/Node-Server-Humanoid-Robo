...
async function run_model(input) {
    const model = await ort.InferenceSession.create("best.onnx");   // To create the model from the points values(as like from pt values in ultralytics YOLO)
...
async function process_output(output, img_width, img_height) {
    ...
    boxes = boxes.filter(box => box[5] > 0.4)
    const result = [];
